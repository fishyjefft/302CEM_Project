  ...
  <body>	
    ...
    <article>
	  ...
	  <?php 
		...
		$query = 'select * from evalley_product where Category=\'electronic\' order by Productid ASC;';
	...
	<tbody>
		...
		if($have_data == true)
		{
			foreach($data as $row)
			{
				...
				echo '<td><a class="btn btn-primary" href="Base/add_cart_item.php?id='.$row['Productid'].'&i_name='.$row['Name'].'&price='.$row['Price'].'&pic='.$row['Image'].'">Add To Cart</a></td>';
